...
import com.small.crawler.util.document.HttpClientFactory;
import com.small.crawler.util.document.HttpURLConnectionFactory;

public class ProxySpirits {
	...
	public static String getUsefulProxy() {
		...
		if (tempProxy == null) {
			...
			if (documentStr.contains("已用完")) {
				...
				String cookie = HttpClientFactory.simulateLoginAcquireCookie(
						"http://http.zhiliandaili.com/Users-login.html", "username", "data_xinheng", "password",
						"data410");
				if (cookie == null) {
					return null;
				}
		...
		String documentStr = HttpURLConnectionFactory.getDocumentStr(param, tempProxy.split(":")[0],
				tempProxy.split(":")[1]);
